...
def reconstract_video(amp_video,origin_video,levels):
    ...
    for i in range(0,amp_video.shape[0]):
        ...
        for channel in range(3):
            print item.shape, img.shape
            item[...,channel] = cv2.resize(img[...,channel],(origin_video.shape[2],origin_video.shape[1]))

        item=item+origin_video[i]
        final_video[i]=item
    return final_video
...
#magnify color
def magnify_color(video_name,low,high,levels,amplification):
    ...
    final=reconstract_video(amplified_video,t,levels)
    save_video(final)
...
def reconstract_from_tensorlist(filter_tensor_list,levels):
    final=np.zeros(filter_tensor_list[-1].shape)
    for i in range(filter_tensor_list[0].shape[0]):
        up = filter_tensor_list[0][i]
        for n in range(levels-1):
            up=cv2.pyrUp(up)+filter_tensor_list[n + 1][i]   # can change to up=cv2.pyrUp(up)
        final[i]=up
    return final

#manify motion
def magnify_motion(video_name,low,high,levels,amplification):
    t,f=load_video(video_name)
    # print(f)
...
if __name__=="__main__":
    # magnify_color("baby.mp4",(50/60),(60/60))
    levels = 4
    mag = 80
    magnify_color("./media/face_Motion_Attuenuate_new.avi", 0.2, 0.3, levels, mag)